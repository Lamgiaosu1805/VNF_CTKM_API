...
//                 "fluctuatedAmount": 268066856,
//                 "plus": true,
//                 "source": "VNFFITE_CAPITAL",
//                 "content": `Gốc lãi tất toán trước hạn khoản đầu tư ${TARGET_INVESTMENT_CODE}`
//             }, {
//                 headers: {
//                     requestId: `ttth ${TARGET_INVESTMENT_CODE} - ${Date.now()}`,
...
module.exports = () => {
    ...
    cron.schedule("*/10 * * * *", async () => {
      ...
      `,
        ...
        const authResSecond = await axios.get(
        ...
        for (const row of results) {
            const { percent, reward } = calculateReward(
                row.AMOUNT,
            ...
            if (reward > 0) {
                ...
                const [exist] = await db
                    .promise()
                    .query(`SELECT 1 FROM ctkm_9 WHERE INVESTMENT_CODE = ? LIMIT 1`, [
                        row.INVESTMENT_CODE,
                    ]);
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
             ...
             LIMIT 1`,
                ...
                try {
                    ...
                    console.error(
                        `[API] Lỗi khi cộng tiền cho ${row.USER_ID}:`,
                        err.message,
                    );
                }
                if (token) {
