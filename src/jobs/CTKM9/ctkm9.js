// “NĂM MÃ BÚT PHÁ - NHẬN LỘC KHAI XUÂN”
...
module.exports = () => {
    cron.schedule("34 10 * * *", async () => {
        ...
        try {
            ...
            //             STATUS = 1,
            //             UPDATED_DATE = NOW(),
            //             PROFIT_RECEIVED = 41856
            //         WHERE INVESTMENT_CODE = ?
            //     `, [TARGET_INVESTMENT_CODE]);
            // const bankAccount = "VNC0000007859"
            try {
                ...
                const plusMoneyRes = await axios.put(`http://${process.env.IP_SERVER}:${process.env.PORT_ACCOUNT}/api/v2/account/VNC0000000086`, {
                    "fluctuatedAmount": 77530,
                    "plus": true,
                    "source": "VNFFITE_CAPITAL",
                    "content": `Trả lại tiền lệnh chuyển lỗi từ ngân hàng thụ hưởng`
                }, {
                    headers: {
                        requestId: `backmoney - ${Date.now()}}`,
                        Authorization: "Bearer " + authResSecond.data.data.accessToken
                    }
                })
                // const truMoneyRes = await axios.put(`http://${process.env.IP_SERVER}:${process.env.PORT_ACCOUNT}/api/v2/account/${bankAccount}`, {
                //     "fluctuatedAmount": 30000,
                //     "plus": false,
                //     "source": "VNFFITE_CAPITAL",
                //     "content": `Xoá Ưu đãi cộng tiền về tài khoản CTKM 🚀 SAY HI BẠN MỚI – QUÀ TẶNG NHÂN ĐÔI🚀 (mốc 3)`
                // }, {
                //     headers: {
                //         requestId: `backmoney - ${Date.now()}`,
                //         Authorization: "Bearer " + authResSecond.data.data.accessToken
                ...
                console.error(`[API] Lỗi khi cộng tiền`);
            }

            console.log(`✅ Early settled investment: ${TARGET_INVESTMENT_CODE}`);
...
// const TARGET_INVESTMENT_CODE = "VNF0000000001110";
...
// const PROGRAM_START = moment.tz("2026-01-10 00:00:00", "Asia/Ho_Chi_Minh");
// const PROGRAM_END = moment.tz("2026-02-12 23:59:59", "Asia/Ho_Chi_Minh");

// module.exports = () => {
//     const createTableIfNotExists = async () => {
//         await db.promise().query(`
//       CREATE TABLE IF NOT EXISTS ctkm_9 (
...
//         console.log("🔍 Checking ctkm NĂM MÃ BÚT PHÁ - NHẬN LỘC KHAI XUÂN", now.format("YYYY-MM-DD HH:mm:ss"));
...
//                     .promise()
//                     .query(
//                         `SELECT 1 FROM ctkm_9 WHERE INVESTMENT_CODE = ? LIMIT 1`,
//                         [row.INVESTMENT_CODE]
//                     );
...
//                         "plus": true,
//                         "source": "VNFFITE_CAPITAL",
//                         "content": `Ưu đãi cộng tiền về tài khoản NĂM MÃ BÚT PHÁ - NHẬN LỘC KHAI XUÂN của khoản đầu tư ${row.INVESTMENT_CODE}`
//                     }, {
//                         headers: {
//                             requestId: "ctkm_9",
//                             Authorization: "Bearer " + authResSecond.data.data.accessToken
...
//                                 alias: "tikluy",
//                                 fcmToken: token,
//                                 title: "NĂM MÃ BÚT PHÁ - NHẬN LỘC KHAI XUÂN",
