...
async function payRewards() {
    ...
    if (rows.length === 0) {
        console.log("[CTKM5] Không có reward nào cần trả thưởng hôm nay");
        return;
    }
    ...
    for (const reward of rows) {
        try {
            ...
            if (bankAccount) {
                try {
                    const plusMoneyRes = await axios.put(
                        ...
                        {
                            fluctuatedAmount: reward.REWARD_MONEY,
                            plus: true,
                            source: "VNFFITE_CAPITAL",
                            content: `Ưu đãi cộng tiền về tài khoản CTKM TIKLUY cùng Nàng – Gửi ngàn yêu thương`,
                        },
                        {
                            headers: {
                                requestId: "ctkm_5",
                    ...
                    if (plusMoneyRes?.data?.result?.isOK !== true) {
                        ...
                        console.log(
                            `💰 Đã cộng ${reward.REWARD_MONEY} vào tài khoản ${bankAccount} (user ${reward.USER_ID})`
                        );
                    }
                } catch (err) {
                ...
                try {
                    await axios.post(
                        "https://service.vnfite.com.vn/push-notification/v2/notification/pushNotification",
                        {
                            alias: "tikluy",
                            fcmToken: token,
                            title: "TIKLUY cùng Nàng – Gửi ngàn yêu thương",
         ...
         WHERE ID=?`,
            ...
            console.log(
                `✅ Hoàn tất trả thưởng user ${reward.FULL_NAME}`
            );
        } catch (err) {
